...
from ms_fuser_variants import get_fuser
...
class MSTemba(nn.Module):
    def __init__(self,
        ...
        self.head_drop = nn.Dropout(p=head_drop)
        ...
        # MS-Fuser: selectable fusion strategy for ablation study
        self.fuser = get_fuser(
            fuser_variant,
            dims=embed_dims,
            E=embed_dims[-1],
            d_state=d_state,
    ...
    def forward(self, x):
        ...
        for i, block_out in enumerate(block_outputs):
            block_pred = self.block_heads[i](self.head_drop(block_out))
            block_predictions.append(block_pred)

        # MS-Fuser: project + fuse the three scale outputs, then run SSM_final
        x1, x2, x3 = concat_x
        x, _ = self.fuser(x1, x2, x3)
