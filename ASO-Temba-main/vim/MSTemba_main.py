...
parser.add_argument('--head-drop', type=float, default=0.0, metavar='PCT',
                    help='Dropout rate before classification heads (default: 0.0)')
parser.add_argument('--label-smooth-eps', type=float, default=0.0, metavar='EPS',
                    help='Label smoothing epsilon for BCE loss; 0 disables (default: 0.0)')
parser.add_argument('--flip-ratio', type=float, default=0.0, metavar='PCT',
                    help='Probability of flipping temporal sequence during training (default: 0.0)')
...
def load_data(train_split, val_split, root, fps_override=None):
    # Load Data
    print('load data', root)

    if len(train_split) > 0:
        dataset = Dataset(train_split, 'training', root, batch_size, classes, int(args.num_clips), int(args.skip), fps_override=fps_override)
    ...
    val_dataset = Dataset(val_split, 'testing', root, batch_size, classes, int(args.num_clips), int(args.skip), fps_override=fps_override)
...
def run(models, criterion, num_epochs=50):
    ...
    # Calcola pos_weight dal training set (una volta sola) per weighted BCE
    logging.info("Calcolando pos_weight per weighted BCE...")
    all_labels = []
    for model, gpu, dataloader, optimizer, sched, model_file in models:
        for data in dataloader['train']:
            all_labels.append(data[2].numpy())
        break  # usa solo il primo modello
    all_labels = np.concatenate(all_labels, axis=0)  # N x T x C
    all_labels_flat = all_labels.reshape(-1, all_labels.shape[-1])  # (N*T) x C
    pos_counts = all_labels_flat.sum(axis=0) + 1e-6
    neg_counts = all_labels_flat.shape[0] - pos_counts
    pos_weight = torch.tensor(neg_counts / pos_counts, dtype=torch.float32)
    pos_weight = pos_weight.clamp(max=50.0)
    logging.info(f"pos_weight: min={pos_weight.min():.2f}, max={pos_weight.max():.2f}, mean={pos_weight.mean():.2f}")
    ...
    for epoch in range(num_epochs):
        ...
        for model, gpu, dataloader, optimizer, sched, model_file in models:
            ...
            train_map, train_loss, block_train_maps, avg_diversity_loss = train_step(model, gpu, optimizer, dataloader['train'], epoch, pos_weight=pos_weight.cuda(gpu))
            logging.info(f'Epoch {epoch} - Train MAP: {train_map:.2f}, Train Loss: {train_loss:.4f}')
            
            # Validation step with block metrics
            prob_val, val_loss, val_map, sample_val_map, block_val_maps, block_sample_val_maps = val_step(model, gpu, dataloader['val'], epoch, pos_weight=pos_weight.cuda(gpu))
...
def run_network(model, data, gpu, epoch=0, baseline=False, pos_weight=None):
    # 
    ...
    # Soft-label smoothing: converts hard 0/1 targets to (eps, 1-eps) when enabled
    if args.label_smooth_eps > 0:
        labels = labels * (1 - args.label_smooth_eps) + args.label_smooth_eps * (1 - labels)

    # Compute loss for final output (weighted BCE con pos_weight per class imbalance)
    loss_f = F.binary_cross_entropy_with_logits(
        outputs_final, labels,
        pos_weight=pos_weight,
        reduction='none'
    )
    loss_f = (loss_f * mask.unsqueeze(2)).sum() / (mask.sum() * labels.shape[-1] + 1e-6)
    ...
    for block_output in block_outputs:
        ...
        block_loss = F.binary_cross_entropy_with_logits(
            block_output, labels,
            pos_weight=pos_weight,
            reduction='none'
        )
        block_loss = (block_loss * mask.unsqueeze(2)).sum() / (mask.sum() * labels.shape[-1] + 1e-6)
...
def train_step(model, gpu, optimizer, dataloader, epoch, pos_weight=None):
    model.train(True)
    tot_loss = 0.0
    ...
    for data in dataloader:
        optimizer.zero_grad()
        num_iter += 1
        outputs, loss, probs, err, block_probs, block_losses = run_network(model, data, gpu, epoch, pos_weight=pos_weight)
        with torch.no_grad():
            inputs, mask, labels, other, hm = data
...
def val_step(model, gpu, dataloader, epoch, pos_weight=None):
    model.train(False)
    apm = APMeter()
    sampled_apm = APMeter()
    block_apms = [APMeter() for _ in range(3)]  # One for each block
...
if __name__ == '__main__':
    ...
    if args.dataset == 'charades':
        ...
        train_split = '/home/3194064/ASO-Temba/data/TSU/Annotation_MS-TEMBA_smarthome-10fps.json'
        test_split = train_split
        rgb_root =  args.rgb_root 
        flow_root = '/flow_feat_path/' # optional
        classes = 53
    ...
    if args.mode == 'flow':
        ...
        dataloaders, datasets = load_data(train_split, test_split, flow_root, fps_override=10 if args.dataset == 'tsu' else None)
    elif args.mode == 'rgb':
        print('RGB mode', rgb_root)
        dataloaders, datasets = load_data(train_split, test_split, rgb_root, fps_override=10 if args.dataset == 'tsu' else None)
