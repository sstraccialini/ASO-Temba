...
def benchmark_eval(args):
    ...
    if args.config_file.endswith(".yaml"):
        ...
        if args.resolution > cfg.model.backbone.square_pad:
            cfg.model.backbone.square_pad = args.resolution
        if args.resolution > cfg.model.backbone.net.img_size:
            cfg.model.backbone.net.img_size = args.resolution
        ...
        cfg.dataloader.num_workers = 0
        if args.benchmark:
            cfg.dataloader.benchmark['mapper']['augmentations'][0]['shape'] = args.resolution
            cfg.dataloader.benchmark.total_batch_size = args.batch_size
            data_loader = instantiate(cfg.dataloader.benchmark)
        else:
    ...
    with tqdm.tqdm(total=max_iter) as pbar:
        for idx, d in enumerate(f()):
            ...
            model(d)
            # print(idx)
            # print(d[0]['image'].shape)
            pbar.update()
    time_end = timer.seconds()
    logger.info("{} iters in {} seconds.".format(max_iter, time_end))
    logger.info("FPS: {}".format(max_iter * args.batch_size / time_end))


if __name__ == "__main__":
    parser = default_argument_parser()
    parser.add_argument("--task", choices=["train", "eval", "data", "data_advanced"], required=True)
    parser.add_argument("--benchmark", action="store_true", help="benchmark the dataloader")
